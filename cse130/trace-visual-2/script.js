...
function animateTrace(n) {
    if (n <= sequence.length) {
        if (n < sequence.length) {
            document.querySelector('.code-line:nth-child('+sequence[n][5]+')').classList.toggle('active')
        ...
        if (n > 0) {
            document.querySelector('.code-line:nth-child('+sequence[n-1][5]+')').classList.toggle('active')
        }
        setTimeout(() => {
            animateTrace(n+1)
        }, 1000)
